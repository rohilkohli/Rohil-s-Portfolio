...
export const ScannerCardStream = ({
  ...
  // Inject keyframe animations that Tailwind can't purge
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .scs-animate-glitch { animation: scs-glitch 0.1s infinite linear alternate-reverse; }

      @keyframes scs-scan-pulse {
      ...
      .scs-animate-scan-pulse {
        animation: scs-scan-pulse 1.5s infinite alternate ease-in-out;
      }
    `;
  ...
  useEffect(() => {
    ...
    // Sync mutable ref state with current prop/derived values
    ...
    for (let i = 0; i < particleCount; i++) {
      positions[i * 3] = (Math.random() - 0.5) * containerW * 2;
    ...
    const createScannerParticle = () => ({
      x: containerW / 2 + (Math.random() - 0.5) * 3,
      y: Math.random() * 300,
    ...
    const handleMouseMove = (e: MouseEvent | TouchEvent) => {
      ...
      if (dt > 0) {
        cardStreamState.current.velocity = Math.min(Math.abs(delta / dt), 1200);
    ...
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      const wheelVelocity = Math.min(Math.abs(e.deltaY) * 3, 1200);
      cardStreamState.current.velocity = Math.max(cardStreamState.current.velocity, wheelVelocity);
    ...
    const animate = (currentTime: number) => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        if (positions[i * 3] > containerW / 2 + 100) positions[i * 3] = -containerW / 2 - 100;
      ...
      // Animate scanner 2D particles
      ctx.clearRect(0, 0, containerW, 300);
      ...
      while (scannerParticles.length < Math.floor(currentMaxParticles))
        scannerParticles.push(createScannerParticle());
      while (scannerParticles.length > Math.ceil(currentMaxParticles))
      ...
      scannerParticles.forEach((p) => {
        ...
        p.y += p.vy;
        p.life -= p.decay;
        if (p.life <= 0 || p.x > containerW) Object.assign(p, createScannerParticle());
  ...
  return (
    ...
    >
      ...
      {showControls && (
        <div className="absolute top-2 left-2 z-30 flex gap-2">
          <button
            ...
            className="px-3 py-1 bg-black/60 border border-white/20 text-white text-xs font-mono"
          >
      ...
      {/* Speed indicator */}
      {showSpeed && (
        <div className="absolute top-2 right-2 z-30 px-3 py-1 bg-black/60 border border-white/20 text-white text-xs font-mono">
          {speed} px/s
        </div>
      )}
      ...
      <div
        className={[
          ...
          'transition-opacity duration-300 z-20 pointer-events-none scs-animate-scan-pulse',
        ...
        >
          {cards.map((card) => (
            <div key={card.id} className="card-wrapper relative w-[400px] h-[250px] shrink-0">
              {/* Image layer — clipped from right as scanner passes */}
              <div
              ...
              >
                <img
                  src={card.image}
                  alt="Project preview"
                  className="w-full h-full object-cover rounded-[15px] brightness-110 contrast-110 shadow-[inset_0_0_20px_rgba(0,0,0,0.1)] hover:brightness-125 hover:contrast-125 transition-all duration-300 ease-in-out"
                />
              </div>
              ...
              >
                <pre
                  className="ascii-content absolute top-0 left-0 w-full h-full text-[rgba(220,210,255,0.6)] font-mono text-[11px] leading-[13px] overflow-hidden whitespace-pre m-0 p-0 text-left align-top box-border scs-animate-glitch"
                  style={{
                    maskImage:
