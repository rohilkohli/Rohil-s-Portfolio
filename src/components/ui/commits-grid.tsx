...
export const CommitsGrid = ({ text }: { text: string }) => {
  ...
  return (
    <section
      className="w-full max-w-xl bg-cyber-gray-900 border border-white/5 grid p-1.5 sm:p-3 gap-0.5 sm:gap-1 rounded-[10px]"
      style={{
      ...
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={cn(
              `border border-white/5 h-full w-full aspect-square rounded-[2px]`,
              isHighlighted ? "animate-highlight" : "",
              shouldFlash ? "animate-flash" : "",
              !isHighlighted && !shouldFlash ? "bg-transparent" : ""
            )}
            style={
              {
